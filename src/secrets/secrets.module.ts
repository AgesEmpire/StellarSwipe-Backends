import { Module } from '@nestjs/common';
import { EventEmitterModule } from '@nestjs/event-emitter';
import { ConfigModule } from '@nestjs/config';
import { RotationService } from './rotation.service';
import { SecretsLoaderService } from './secrets-loader.service';
import { SecretRotationScheduler } from './rotation-scheduler.service';
import { CredentialOverlapService } from './rotation/credential-overlap.service';
import { RotationEventService } from './rotation/rotation-event.service';

@Module({
  imports: [EventEmitterModule.forRoot(), ConfigModule],
  providers: [
    RotationService,
    SecretsLoaderService,
    SecretRotationScheduler,
    CredentialOverlapService,
    RotationEventService,
  ],
  exports: [
    RotationService,
    SecretsLoaderService,
    SecretRotationScheduler,
    CredentialOverlapService,
    RotationEventService,
  ],
})
export class SecretsModule {}
